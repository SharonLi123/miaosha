...
public class GlobalExceptionHandler {
    ...
    public String exceptionHandler(HttpServletRequest request,Exception e){
        if (e instanceof GlobalException){
            ...
            return JSON.toJSONString(Result.error(globalException.getCodeMessage()));
        } else if (e instanceof BindException){
            BindException bindException=(BindException)e;
            List<ObjectError> objectErrors=bindException.getAllErrors();
            ObjectError error=objectErrors.get(0);
            String msg=error.getDefaultMessage();
            return JSON.toJSONString(Result.error(CodeMessage.BIND_ERROR.fillArgs(msg)));
        }else {
            return JSON.toJSONString(Result.error(CodeMessage.SERVER_ERROR));
        }
